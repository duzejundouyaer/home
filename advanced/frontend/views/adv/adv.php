...
<div class="panel admin-panel">
  ...
  <table class="table table-hover text-center">
   ...
   <?php foreach ($data as $key => $value): ?>
    <tr>
      <td><?=$value['adv_id']?></td>     
      <td><img src="upload/<?=$value['adv_img']?>" alt="" width="120" height="50" /></td>     
      <td><?=$value['adv_title']?></td>
      <td><?=$value['adv_desc']?></td>
...
<div class="panel admin-panel margin-top" id="add">
  ...
  <div class="body-content">
    <form method="post" class="form-x" action="<?=Url::toRoute(['adv/adv_add'])?>" enctype="multipart/form-data">     
...
<div class="panel admin-panel margin-top" id="update">
  <div class="panel-head"><strong><span class="icon-pencil-square-o"></span>修改内容</strong></div>
  <div class="body-content">
    <form method="post" class="form-x" action="<?=Url::toRoute(['adv/adv_update_one'])?>" enctype="multipart/form-data">     
      <div class="form-group">
        <div class="label">
          <label>标题：</label>
        </div>
        <div class="field">
          ...
          <input type="hidden" value="" id="hidden" name="adv_id">
          <div class="tips"></div>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" name="url" value="" id="url" />
          <div class="tips"></div>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <div class="field">
          <textarea type="text" class="input" name="note" style="height:120px;" value="" id="desc"></textarea>
          <div class="tips"></div>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" name="sort" value="" id="sort"  data-validate="required:,number:排序必须为数字" />
          <div class="tips"></div>
        </div>
      </div>
      <div class="form-group">
...
<script type="text/javascript">
  $(function(){
      $(document).on("click","#updateAdv",function(){
            var advId = $(this).parent().attr("id");
            $.ajax({
             type: "POST",
             url: "?r=adv/update_adv",
             data: {id:advId},
             dataType:"json",
             success: function(msg){
                // console.log(msg.adv_id);
                //alert(msg.adv_id)
                $("#hidden").val(msg.adv_id);
                $("#title").val(msg.adv_title);
                $("#url").val(msg.adv_url);
                $("#desc").val(msg.adv_desc);
                $("#sort").val(msg.adv_sort);
             }
          });
      })
